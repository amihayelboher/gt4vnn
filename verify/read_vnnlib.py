'''
using the code of Stanley Bak (June 2021) for vnnlib simple utilities
'''
...
@dataclass
class Property:
    # 2 arrays: 1 for lowers, 1 for uppers
    input_constraints: Tuple[np.array, np.array]  
    output_constraints: Tuple[np.array, np.array]
...
if __name__ == "__main__":
    ...
    benchmark_dir = "/home/yizhak/Research/Code/vnncomp2022_benchmarks/benchmarks/mnist_fc"
    benchmark_path = Path(benchmark_dir)
    prop_path = benchmark_path / "vnnlib/prop_0_0.03.vnnlib"
    onnx_path = benchmark_path / "onnx/mnist-net_256x6.onnx"
    num_inputs, num_outputs, _ = get_num_inputs_outputs(onnx_path)
    vnnlib = read_vnnlib_simple(prop_path, num_inputs, num_outputs)
    # print(vnnlib[0])
    input_lower_bs = np.array(vnnlib[0][0])[:, 0]
    input_upper_bs = np.array(vnnlib[0][0])[:, 1]
